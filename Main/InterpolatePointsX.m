function InterpolatePointsX(app,selectedValues,drawFigures,stepsForFigures,j,i,Side)
...
if Side =="Narrow"
    ...
    app.Distances.(selectedValues{i}).PointsX(j+1,1) = {realDistancePointsc};
    app.Distances.(selectedValues{i}).PointsX(j+1,2) = {realDistancePointsa};
...
if Side =="Broad"
    ...
    app.Distances.(selectedValues{i}).PointsX(j+1,3) = {realDistancePointsc};
    app.Distances.(selectedValues{i}).PointsX(j+1,4) = {realDistancePointsa};
    %% Draw Figures
    if drawFigures==true && mod(j,stepsForFigures)==0
        fig = figure('visible','off',...
            'Position',[500 300 500 700]);
